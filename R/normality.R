...
write_norm<-function(object, path, plots=FALSE, tablefont="Arial", tablefontsize=10, digits=NULL){
  ...
  doc <- read_docx()
  doc %>% body_add_fpar(fpar(ftext('Normality Tests', prop=format_table_title)), style='centered')
  doc %>% body_add_par("")
  ...
  for(i in 1:length(object$output)){
    ...
    ft <- colformat_num(x=ft, digits = digits, col_keys = names(object$output[[i]]), na_str = "NA")
  ...
  if(plots==TRUE){
    ...
    for(i in 1:length(plt)){
      doc <- body_add_gg(doc, ggplotify::as.ggplot(plt[[i]]))
      doc %>% body_add_par("")
    }
  }
